...
function setupModalListeners() {
    ...
    // Modal de edição
    document.getElementById('closeEditParticipantModal').addEventListener('click', closeEditParticipantModal);
    document.getElementById('cancelEditParticipant').addEventListener('click', closeEditParticipantModal);
    document.getElementById('deleteParticipant').addEventListener('click', handleDeleteParticipant);
    
    // Fechar modais clicando fora
    [participantModal, weightModal, statsModal, historyModal, editParticipantModal].forEach(modal => {
        ...
        return `
            <div class="participant-card">
                ...
                <div class="participant-actions">
                    <button class="btn btn-primary" onclick="editParticipant('${participant.id}')">
                        <i class="fas fa-edit"></i> Editar
                    </button>
                    <button class="btn btn-info" onclick="viewParticipantHistory('${participant.id}')">
...
function editParticipant(participantId) {
    ...
    document.getElementById('editInitialWeight').value = participant.initialWeight;
    document.getElementById('editTargetWeight').value = participant.targetWeight || '';
...
function handleEditParticipantSubmit(e) {
    e.preventDefault();
    ...
    // Atualizar peso atual se o peso inicial mudou
    if (participants[participantIndex].initialWeight !== updatedData.initialWeight) {
        participants[participantIndex].currentWeight = updatedData.initialWeight;
    }
...
function handleDeleteParticipant() {
    ...
    // Confirmar exclusão
    if (confirm(`Tem certeza que deseja excluir o participante "${participant.name}"?\n\nEsta ação não pode ser desfeita e todos os dados de pesagem serão perdidos.`)) {
        // Remover participante
        participants = participants.filter(p => p.id !== participantId);
        
        // Remover registros de peso do participante
        weightRecords = weightRecords.filter(r => r.participantId !== participantId);
...
// Visualizar histórico de um participante específico
function viewParticipantHistory(participantId) {
    ...
    // Filtrar pelo participante selecionado
    setTimeout(() => {
        document.getElementById('historyParticipantFilter').value = participantId;
        filterHistory();
    }, 100);
}
